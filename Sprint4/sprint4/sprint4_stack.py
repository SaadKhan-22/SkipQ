...
class Sprint4Stack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        WH_duration = WH_lambda.metric('Duration' , period = Duration.minutes(60))
        WH_invocations = WH_lambda.metric('Invocation' , period = Duration.minutes(60))

        WHdurAlarm = cloudwatch_.Alarm( self , "WHLambdaDurationAlarm",
            threshold= 4750, #in milliseconds
            evaluation_periods=1,
            metric = WH_duration
             )
        WHdurAlarm.apply_removal_policy(RemovalPolicy.DESTROY)

        WHinvocAlarm = cloudwatch_.Alarm( self , "WHLambdaInvocationAlarm",
            threshold= 1,
            evaluation_periods=1,
            metric = WH_invocations 
             )
        WHinvocAlarm.apply_removal_policy(RemovalPolicy.DESTROY)

        #https://docs.aws.amazon.com/cdk/api/v2/python/aws_cdk.aws_lambda/Alias.html

        version = WH_lambda.current_version
        alias = lambda_.Alias(self, "SaadkWHLambda_Alias", alias_name="SaadK_WH_Prod", version=version)
        ...
        WH_lambda_deployment = codeDeploy.LambdaDeploymentGroup(self, "SaadKwebHealthLambdaDeployment",
            alias=alias,
            alarms=[WHdurAlarm, WHinvocAlarm],
...
#https://docs.aws.amazon.com/cdk/api/v2/python/aws_cdk.aws_cloudwatch/Alarm.html
        ...
        CRUD_lambda = self.create_lambda("CRUDLambda", "./resources", "CRUD_lambda.lambda_handler", Lambda_role)
        CRUD_lambda.apply_removal_policy(RemovalPolicy.DESTROY)


        URLtable = db.Table(self, "URL_Table",
            partition_key = db.Attribute(name="id", type = db.AttributeType.STRING)
                    )
        URLtable.apply_removal_policy(RemovalPolicy.DESTROY)

        Tablename = URLtable.table_name
        CRUD_lambda.add_environment(key='CRUD', value = Tablename)



        #https://docs.aws.amazon.com/cdk/api/v1/python/aws_cdk.aws_apigateway.html
        API = api.LambdaRestApi(self, "SKsprint4Api",
            handler = CRUD_lambda,
            proxy = False
                    )
        ...
        resource = API.root.add_resource("urls")

        resource.add_method('POST')

        resource.add_method('GET')

        resource.add_method('DELETE' )

        resource.add_method('PATCH')

        resource.apply_removal_policy(RemovalPolicy.DESTROY)

        self._api_url = CfnOutput(self, 'SaadK_API_URL', value= API.url, description="The generated API URL used to call the API Lambda")
